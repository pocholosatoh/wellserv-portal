...
export default async function FollowUpCard() {
  ...
  return (
    <div
      className="group relative overflow-hidden rounded-2xl border border-white/70 bg-white/80 shadow-[0_18px_45px_rgba(15,23,42,0.08)] ring-1 ring-black/0 transition hover:-translate-y-0.5 hover:shadow-[0_26px_60px_rgba(15,23,42,0.12)]"
    >
      ...
      <div className="flex items-center justify-between px-5 pt-5 sm:px-6">
        ...
        <div className="flex items-center gap-2 text-xs">
          {rightBadge && (
            <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-medium shadow-sm ${rightBadge.cls}`}>
              {rightBadge.text}
            </span>
          )}
      ...
      <div className="space-y-3 px-5 pb-5 pt-4 text-sm text-slate-600 sm:px-6">
        {!f && (
          <>
            ...
            <div className="text-xs text-slate-500">If you were advised to return, please call the clinic.</div>
          </>
        )}
        ...
        {f && (
          <>
            <div className="text-slate-700">
              <span className="font-semibold text-slate-800">When:</span> {fmtManilaDate(f.due_date)}
            </div>
            <div className="text-slate-700">
            ...
            {f.intended_outcome && (
              <div className="text-slate-700">
                <span className="font-semibold text-slate-800">Doctor’s goal:</span> {f.intended_outcome}
              </div>
            )}
          </>
        )}
