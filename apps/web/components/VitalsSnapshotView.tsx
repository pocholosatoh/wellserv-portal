...
export type VitalsSnapshotViewData = {
  ...
  blood_glucose_mgdl?: number | string | null;
  notes?: string | null;
  source?: string | null;
...
function normalizeSnapshot(row: VitalsSnapshotViewData, index: number): NormalizedVitals {
  const fallbackId = row.id ?? row.measured_at ?? row.created_at ?? index;
  return {
...
export default function VitalsSnapshotView({
  snapshots,
}: {
  snapshots?: VitalsSnapshotViewData[] | null;
}) {
  ...
  const history = latest ? normalized.filter((snap) => snap.id !== latest.id) : normalized;
